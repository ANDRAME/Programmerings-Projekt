...
static uint32_t g_invincible_until_hs[3];
...
static void spawn_powerup(Bounds *bounds)
{
    ...
    uint32_t r = lcg_next();
    uint32_t pick = (r >> 16) % 3;
    g_pu.type = (pick == 0) ? POWERUP_TRIPLE_SHOT
             : (pick == 1) ? POWERUP_AUTO_FAST
                           : POWERUP_INVINCIBLE;
...
void powerups_init(void)
{
    ...
    g_invincible_until_hs[1] = g_invincible_until_hs[2] = 0;


    g_pu.next_spawn_hs = time_now_hs() + 200u;
}
...
uint8_t powerup_invincible_active(uint8_t player)
{
    if (player < 1 || player > 2) return 0;
    uint32_t now = time_now_hs();
    return ((int32_t)(g_invincible_until_hs[player] - now) > 0);
}
