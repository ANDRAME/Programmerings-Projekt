...
LorentzParams_t lorentz_defaults(void)
{
    ...
    p.soft2  = 36;      // ~6px
    p.range2 = 25*25;   // only within 25px
    p.radius = 5;       // asteroid radius
    p.edge   = 5;       // only near the side
...
int  physics_step_bullet_lorentz(
    ...
    // kill prevent circling back
    if (b->vy >= 0) return 0;
    if (b->vx > FX16_FROM_INT(3) || b->vx < -FX16_FROM_INT(3)) return 0;
